...
Based on the provided discussion in Chapter \ref{c:related} we have established six \todoMissing{fill} fundamenetal functional
...
\todoMissing{Consistency does not only resolve around ensuring the consistenc yof all replicas  but also on consitency in term sof processing and handling within one transaction
This could mean: how a system handles freshness within a transaciton how locks are applied and queries are shielded against each other}
...
\todoMissing{Disallow DML operations when freshness queries will be or have been executed within the same transaction. Which would violate the ACID properties.}
