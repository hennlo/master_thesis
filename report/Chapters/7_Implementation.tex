...
This flexibility leverages Polypheny-DB towards a data orchestration platform and an actual PolyDBMS~\cite{polypheny2021}. 
...
In correspondence to the Section \ref{sec:propagation} update propagtion within concept we focussed on implementing the CDC approach ($\rightarrow$ Section \ref{sec:algo}) 
...
targeted to an entire data placement (Listing \ref{lst:state}).
...
The newly introduce \emph{Replication Engine}, contains the core functionality that transforms capture objects into distinct replication objects and 
