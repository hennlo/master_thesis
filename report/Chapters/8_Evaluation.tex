...
In general we aim to assess different situation to obtain a comprehensive insights on the introduced implementations.
...
To further assist the execution of the benchmark tests, a number of tools are used to standardize and ease the evaluation procedure.
...
Furthermore it shows that again with a growing number of stores, the gap between the old and new locking extends even more, validating the benefits of the new locking mechanism, if used within strongly partitioned
configurations.
...
This illustrated comparison clearly shows that due to its limited resources the PostgreSQL store cannot directly compete with this HSQLDB store.
This provides us with the intuitive decission to use HSQLDB for the primary transactions.
...
For this test we are using a Mixed Workload to allow permanent access to the primary node, while the lazy replicated nodes can solely concentrate on applying the queued changes.
...
require the change. As summarized in Figure \ref{fig:converge24} this directly influences the earlier described convergence time of our system and negativiely impacts the response time.
...
On the basis of fully freshness optimized read-operations using an index of $0.7$, 
